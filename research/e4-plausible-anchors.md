...
> declared central-favourable endpoint `PRO_CENTRAL` a **narrow central win** (coverage m ≈ −2.3%), with a
...
| `pi_opp,mu_opp` opposition prevalence & intensity | E | contract | band | Einstein–Palmer–Glick 2019 (*Neighborhood Defenders*: local hearings dominated by an **intense, unrepresentative minority**) | anchored (empirics) | ✅ ANCHORED |
| `s_exp,b_H_C` harm myopia (HEADLINE) | T | contract | band `s_exp∈[1,2.5], b_H_C∈[0.5,1.3]` (stylized mag.) | **over-determined**: Hayek 1945 (knowledge problem); Scott 1998 (legibility, *Seeing Like a State*); Olson 1965 + Schattschneider 1935 + Wilson 1980 (diffuse costs under-weighted); Bandiera–Prat–Valletti 2009 (**83% passive waste**, standardized-goods procurement); Bastiat 1850; Bachrach–Baratz 1962. (Flyvbjerg belongs to projection/optimism, NOT the low-visibility harm gate.) | **direction strongly anchored (theory+empirics)**, magnitude stylized+swept | ✅ ANCHORED |
| `w,a,b` projection / bias | P | contract | band | Broockman–Skovron 2018 (**3,765 politicians dramatically overestimate** constituents' conservatism); Flyvbjerg 2014 (**benefits systematically over-rated**); Dias–Lucas–Sheffer; Gagnon-Bartsch (~21–50% projection) | strong (dir.+mag. band) | ✅ ANCHORED |
...
| `beta` voice suppression | T | contract | band | Noelle-Neumann 1974 (spiral of silence); Einstein–Palmer–Glick 2019 (who speaks ≠ who is affected); Olson 1965 (diffuse under-mobilized) | anchored; used as a distributed HANDICAP (conservative) | ✅ ANCHORED |
...
> (`PRO_CENTRAL`) is **a narrow central win (coverage ~-2.3%)**; a central-winning region exists (~9/40 draws 20% inward).
...
|---|---|---|---|---|---|
| PRO-CENTRAL (central's declared full-favourable endpoint) | **−2.3%** | [−2.5, −2.2] | 95.3% of the reference | 97.6% | narrow central win |
...
- **Where each stands (faithful participation):** coverage wins across the anchored region. The central's
  ...
  a narrow central win (~−2.3%), and a central-winning region exists ~20% inward (≈9/40 draws). The story from D/O: Core v0 is robust
...
- **Continuity / reconciliation with the NO-GO:** in this sequential, path-dependent decomposition, `MYOPIA_OFF`
  ...
  competent central narrowly leads only at the fully-idealized endpoint. Qualitative reconciliation across DGPs, not
  a reproduced limit.
...
- **PRO-CENTRAL** — the central's declared full-favourable endpoint (every knob central-favourable). Under faithful
  universal participation this fully-idealized endpoint is **a narrow central win (≈ −2.3%)**, and a central-winning
...
- **Effect of the correction (`e4:scenarios`/`e4:evidence`):** setting `p=1` (from the old 0.05–0.40) collapses the
  central's declared full-favourable endpoint from a ~30-point win to a **narrow central win (~−2.3%)**, and raises PROBABLE
