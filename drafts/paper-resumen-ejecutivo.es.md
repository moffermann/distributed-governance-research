...
- **La arquitectura es el aporte; la ventaja medida es real pero modesta.** Cuando sometimos el mecanismo de selección distribuida a una prueba pre-registrada y simétrica —igualando en esperanza el presupuesto de tasación de ambos brazos (aunque el ruteo de esa información difiere a propósito entre ellos), sin ventajas de partida—, la ventaja fue positiva en todas las celdas pero pequeña, por debajo del umbral de reconstrucción (rebuild) que nos habíamos fijado *de antemano* —un umbral de decisión de investigación sobre una escala no calibrada, no un umbral de materialidad de política. Por eso retiramos el multiplicador grande que una versión anterior había reportado y nos apoyamos en el diseño y en la *dirección* del mecanismo, no en un número. La honestidad sobre ese límite es parte del aporte, no una nota al pie.

- **Lo que más rinde es la entrega verificada, no solo la selección.** Al modelar juntas las dos capas —*quién elige* los proyectos y *cómo se entregan* (el control opaco del statu quo frente a la verificación de Core v0)—, ambas se **componen multiplicativamente**: la entrega verificada amplifica la ganancia de una buena selección, en lugar de simplemente sumarse a ella. En el mundo declarado del modelo, la arquitectura completa de selección y entrega supera al statu quo por un margen amplio —cerca de +58,6 puntos de la referencia interna del modelo, con un intervalo estrecho [+58,0, +59,2]—. Es un contraste condicional del modelo, no un efecto de campo calibrado; pero ubica la ventaja con nitidez en el **valor entregado**, no en la selección por sí sola.

- **La ventaja es el valor entregado, no un ahorro administrativo.** Al contabilizar de forma simétrica el costo de la maquinaria que cada institución opera —la central en estudios de valor, asignación, priorización y licencias; Core v0 en su propia plataforma y su control—, el costo administrativo resulta **aproximadamente neutro bajo un supuesto conservador de baja diferencia**. Si se admite un escenario de costos asimétrico —la burocracia central de tasación y priorización, con sus sueldos, pesa bastante más que una plataforma digital con fiscalización de costo casi nulo—, la dirección favorece a Core v0; pero ese diferencial es un supuesto declarado, no una medición. La eficiencia principal no proviene de recortar gastos generales, sino de que, a igual presupuesto, llega más valor por cada peso.
