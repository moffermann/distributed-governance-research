# Roadmap — full pre-restructure archive (snapshot 2026-07-10)

This is the COMPLETE roadmap as it stood before the 2026-07-10 audit restructure, preserved verbatim so nothing is lost. The live, lean roadmap is `docs/03_ROADMAP.md`. Everything below — the C001-C025/H001-H059 alignment block, the residual-cleanup log #1-#122, the Phase 0-9 template, the superseded 2.09/1.83/2.2x E4/E5 planning sections, and the "Preserve A0xx boundary" invariants — lives here as history.
...
The one-hundred-fifteenth residual cleanup corrected the simulation's framing across the corpus after an author methodological review. The E2 finding formerly phrased as "planner knowledge dominates allocation quality" was rescoped to what the model actually shows — the informational quality of the project prioritization the passive share follows (the aggregated allocation profiles) dominates, whoever or whatever supplies it — and the pre-registered E4 experiment ([[e4-institutional-knowledge-design|research/e4-institutional-knowledge-design.md]]) modeled knowledge symmetrically and found open construction of the vector from aggregated dispersed citizen signals viable and scale-robust, beating fixed-bandwidth central construction at every tested scale under declared preconditions (aggregation institution present; signals honest and unbiased; elicitation non-strategic). Derived statements were aligned in the manuscript (v1.3-v1.4), the publishable model's limitation bullet and tested-so-far section, the blueprint's A020 block, the entity/state map blockquote, and the distributed-roadmap open question's status note: agenda-setting centralization is a property of the closed and tutored transition modes, not of the architecture; the default layer is the pluggable civic autopilot of H033; and the open problem is honest elicitation, not the feasibility of distributed construction.
...
The one-hundred-twenty-second residual cleanup (2026-07-07) consolidated the satellite computational program (Offermann 2026b, doi:10.5281/zenodo.21246089) into the corpus and manuscript: v1.9 applied the queued changes, and the two rules the manuscript now asserts were formalized through the pipeline — [[112_BUDGET_RELEASE_RULE_AND_A042_RESOLUTION|docs/112]] (pull budget release against a verification-calibrated WIP ceiling, A042) and [[113_VERIFICATION_PACKAGE_AND_A043_RESOLUTION|docs/113]] (machine triage with a permanent human second instance, seeded controls, contraposition bounded by contributor independence, and first-class cross-layer collusion resistance, A043). A second internal adversarial review round on the satellite Finding 7 (five reviewer archetypes) found a construct bug in the machine-panel false-flag half and scope overshoot in its framing; the correction (a de-leaked, fixed-decoding, CI-bearing re-run and scope-limiting language) shipped as v1.10 (doi:10.5281/zenodo.21249063), with the satellite corrected in parallel (v0.5, doi:10.5281/zenodo.21249060). Experiment G (collusive adversary) established that cross-layer collusion is the one attack to move leakage by an order of magnitude while the verified-value advantage survives — scoping the verification claims to a non-colluding adversary. The attack record stands at 43/43 resolved.

## Manuscript roadmap (v1.10 → next)

**Corrections applied through v1.10:** the v1.9 change queue is applied; the machine-verification language corrected after the satellite construct bug (the evidence-richness and temperament-spread claims withdrawn; Section 9 now capability-tiered and scoped to document-legible fraud; "court-grade"→"tamper-evident"); cross-layer collusion resistance added as first-class after Experiment G; the release rule and verification package formalized (docs/112, docs/113); every citation surface on the concept DOI so deposited PDFs cannot go stale (see [[RELEASING|RELEASING.md]]). The `drafts/paper-v1.9-queue.md` is marked applied.

**Improvement points (next manuscript revision):**
- **Q4 title rename** "Adversarial Validation"→"Adversarial Review" — the one standing queued item, venue-gated by its own terms.
- **Section 9 maturity labeling**: the machine-verification and collusion material (satellite F7/F8) is the least mature; keep it explicitly flagged and lean on the satellite for depth rather than importing more of it (the consolidation strategy).
- **Value-weighted human-touch and the physical/pre-contract scope**: fold the fiscalizer's scope limits into the pathway prose wherever the human-touch fraction or "court-grade" appears, not only in the satellite.
- **Distributional-outcome metric**: the status quo is scored only on verified delivery; a distributional/equity metric (named future work) would answer the "value-inert incumbent" objection at the manuscript level.

**Pending validation program (the independence the corpus most needs):**
- **External expert review** with the prepared packets (now v1.10, cover note in `external-review/SENDING_COVER_v1.10.md`) — the standing human step; the two highest-value profiles are an adversary-side reviewer (does Experiment G's collusion analysis hold) and an evals-methodology reviewer (does Finding 7 survive a specialist).
- **Empirical calibration**: the D-lite human pilot of the elicitation instrument (satellite `D_LITE_PILOT_PROTOCOL.md`) is the first empirical anchor against the LLM priors; the full Experiment D follows.
- **A bounded tutored pilot** with the instrumentation the resolutions already require.

**Dedicated follow-up studies (candidate second/third manuscripts):**
- **The agenda-setting study** ([[agenda-setting-study-design|knowledge/open-questions/agenda-setting-study-design.md]]) — the parallel-agenda mechanism, semi-open envelope objects, and honest elicitation under gaming; the corpus's largest remaining intellectual object.
- **A collusion-resistance mechanism-design study** following Experiment G: the defenses (verified beneficial ownership, contributor Sybil-resistance, decentralized assigner/seeder/floor-setter) are recorded but unmodeled.

Next review candidates: the post-manuscript validation program — external expert review using the prepared packets (now v1.9/v1.10), calibration to participatory-budgeting data (a bounded human pilot of the elicitation instrument is the first empirical anchor), and a bounded tutored pilot; the agenda-setting study formalizing docs/110's two untreated configurations (the leading candidate for a dedicated second manuscript); a collusion-resistance mechanism-design study following Experiment G; and continued cross-validation of the satellite. The manuscript growth constraint is relaxed under the consolidation strategy: the satellite (citable DOI) absorbs depth, the master cites and states the rules.
