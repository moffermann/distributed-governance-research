...
| Herding indicator | Not a Core v0 primary entity. A027 is observability over existing objects: funding-velocity-versus-evidence-maturity indicators, funding-source independence signals, and discovery-slot concentration read from citizen discovery and project-list ordering (C025), the support-versus-financing separation (H026), and administrative observability, with funding-target closure (H035) retained as the structural cascade truncator. Per the simulation, closure lowers concentration and lifts completion breadth but does not by itself improve quality selection, which lives in the default anchor; blind early windows, staged reveal, and exposure equalization remain Extension v1+. |
| Territorial-equity metric | Not a Core v0 primary entity. A028 is administrative observability over existing objects: funding per capita by comuna, funded-project density against need weights, capture share by organized proposers, and an explicit input-equity versus outcome-equity distinction read from the audit trail, PlanningScope, the allocation formula (H025), and the default rule (H033). Equalization — redistributive and hybrid formulas, planning-scope lanes and the A005 floor-and-lane pattern reused for a territorial or group equity target, territorial routing, and supply-side support — is a versioned, publicly authorized, auditable country-implementation choice, not a Core v0 mandate. |
| Cross-project dependency record | Not a Core v0 primary entity. A029 is coordination over the existing Composite Program object (H051): hard cross-project dependency declarations, program-level funding-coherence visibility, stranded-complement warnings, and dependency-aware funding windows read from project and phase funding attempts and Planning Scope alignment (H009). Components keep independent funding attempts; conditional or escrowed bundle funding, shared procurement, and shared control packages remain Extension v1+, and complementarities can still strand when prerequisites fund and dependents do not. |
| Clientelism pattern indicator | Not a Core v0 primary entity and not an automatic verdict. A030 is observability over existing objects: repeat broker-beneficiary alignments across cycles, geographic delegation blocs correlated with related-project funding, and support-provider concentration read from delegation and the Delegate role (C019), participation-support projects, the related-party relationship graph (A014), and delegated-action reports, feeding review, complaint, and referral paths as leads. The off-platform quid pro quo remains invisible and sanction routes to external law; hard anti-broker bans are not adopted. |
| Motive or polarization record | Not a Core v0 primary entity, and no motive is policed. A031 is a motive-neutrality boundary plus funding-bloc correlation observability over existing objects: social sovereignty over value (P002), value icons and antivalue-ceiling declarations (H012), support and justified-objection signals (H026), the common-good impact declaration (C022), and the funding-target closure rule (H035), which bounds brigade amplitude. Material effects stay gated identically regardless of motive; mobilization-pattern indicators carry no platform effect, and viewpoint moderation and polarization scores are excluded on principle. |
| Intertemporal-allocation metric | Not a Core v0 primary entity. A032 is observability plus optional planning-scope lanes over existing objects: long-horizon lane need-weights flowing into the default allocation, funding share by time-to-benefit horizon, and deferred-maintenance indicators read from Planning Scope (H009), the monthly allocation cycle (H038), continuity-risk classification (A006), and citizen funding and automatic allocation profiles, with the non-assignable pool retained as the structural shield. Discount-correcting preferences, forced intergenerational weights, and multi-cycle or endowment lanes remain Extension v1+. |
| Allocation-act characterization | Not a Core v0 primary entity and not a standing adjudication. A034 is a field set on the `PlanningScope` Allocation Mandate record: a bounded characterization menu (administrative act, sui generis participation act, or preference input), a pilot default of sui generis participation act with vote-like citizen immunity and no individual review right over rule-based reassignments, the complaint path unchanged for rule violations, plain-language citizen-surface disclosure before first allocation, a comparator note on the replaced official act, and versioning with AuditEvents treating any change as a higher-authorization decision. The platform records the jurisdiction's chosen characterization; doctrine construction, standing adjudication, and menu consequences remain external constitutional and administrative law, untested in any jurisdiction until a pilot produces a dispute. |
| Administrative Capacity Declaration | Not a Core v0 primary entity and not a staffing model. A035 is the third versioned per-scope record on `PlanningScope` beside the Allocation Mandate and Fiscal Commitment Profile: expected review volume, staffing assignment, and per-resolution effort assumptions, with timeout policies calibrated to it and never copied between scopes with different declarations, authority-side AI assistance inside the C008 boundary where AI assists and the responsible authority decides, review-window performance joined to the H054 functional transition maturity metrics, and hours-per-resolution measurement by resolution type committed as a first-pilot deliverable. A lapsed deadline produces a public `ReviewTimeoutResolution` naming the responsible authority rather than an anonymous system action; staffing economics, headcount prescription, and deployment cost models remain country implementation, and every declaration is a public, falsifiable assumption until a pilot measures hours per resolution. |
