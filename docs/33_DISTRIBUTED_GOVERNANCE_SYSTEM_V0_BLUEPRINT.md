...
A019 adds an Allocation Mandate record on Planning Scope, not mandate-construction machinery: every active Planning Scope records the democratic mandate that authorized its budget migration and its allocation formula, distinct from the procedural Administrative Rule Change machinery. The minimum record names the mandate source (statute, ordinance, referendum, or delegated authority), the legal instrument authorizing budget migration, the migrated share, the allocation-amount formula version with an explicit flag and authorizing authority when it departs from equal-per-citizen, and effective dates with version history; a non-equal formula is a higher-authorization decision than an ordinary parameter edit. Core v0 records and exposes the mandate but does not create, validate, or adjudicate it — mandate construction remains external constitutional law and country implementation, and the record separates "visible and versioned" from "democratically authorized" so an auditable reallocation can no longer pass as an authorized one.

A020 is bounded, not solved: whoever constructs Planning Scopes exercises the second face of power, deciding what citizens may fund at all before allocation, support, or fiscalization occurs, and Core v0 adds no mechanism that governs who constructs the roadmap. It carries interim observability over existing objects instead: every Planning Scope shows its Allocation Mandate record and a versioned scope definition; scope changes and interpretations surface as Governance Resolutions with declared review windows and timeouts; and out-of-scope demand stays visible as Ideas, with volume observability per rejected or out-of-scope category. Agenda-setting power is thereby documented, versioned, and contestable through visibility only. Distributing scope construction remains the architecture's principal open problem — the acknowledged constitutional gap in the distributed-roadmap-construction open question and a blocker for open-mode deployment at scale — reinforced by the simulation finding that the informational quality of the scope's weight vector dominates allocation quality, and reframed by the pre-registered E4 result: constructing that vector from aggregated citizen signals is viable and scale-robust in the model, so the open problem is the honest-elicitation mechanism, not the feasibility of distributed construction.

A021 adds a Fiscal Commitment Profile on Planning Scope plus fiscal-reliability observability, not any enforcement mechanism: the state still controls the fiscal spigot and can strangle the system quietly — through late or partial delivery of signed balances, mid-cycle reduction of the migrated share, zero indexation, or valid Financial Orders left unexecuted — while the platform shows green funding states. Each active Planning Scope carries a public, versioned Fiscal Commitment Profile recording migrated percentage, indexation rule, delivery-latency target, and cycle horizon, and changes to it are visible governance events with reason, magnitude, affected scopes, and beneficiary impact rather than silent parameter edits. Delivery and execution reliability surface as public signals attributable to the fiscal authority rather than the visible executor: expected-versus-actual delivery of signed balances, order-to-execution latency distribution, an aggregate unexecuted-valid-order indicator, and a real-value-preservation warning where nominal amounts are not inflation-adjusted, with missed windows and systematic delay feeding the H054 transition metrics and the A016 incumbent-resistance indicators. Core v0 records and surfaces fiscal behavior but enforces nothing — no software can compel a sovereign to pay, so multi-year appropriation locks, statutory indexation, and budget floors remain country implementation, and a determined treasury can still defund the system in full public view.

A033 adds a Duty-of-Care Anchor named before disbursement, not a liability-adjudication mechanism: when harm to a third party results from the interaction of adequately-performed roles, the responsibility matrix can truthfully find that no role breached its internal obligation, C012 forbids blame by association, and P003 assigns responsibility to "the project," which is not a legal person a victim can sue. Every execution-financeable project therefore names, before disbursement, the solvent, reachable legal person or persons answerable to third parties for physical and safety harm — an executor, guarantor, insurer, or designated responsible operator — exposed on the citizen-facing sheet as a plain statement such as `responsible party: [name] — liability coverage active`. The Financial Assurance and Post-Closure Coverage profiles extend to third-party liability coverage proportionate to physical risk where Threshold Policy requires it; versioned documentation records that distributed micro-allocation does not dilute the state's underlying duty of care and where residual public responsibility remains; the responsibility matrix distinguishes "no role breached its internal obligation" from "no one is liable to the victim"; and the audit trail preserves a harm-attribution evidence package — roles, versions, gates, evidence — usable by a court. Core v0 records the duty-bearer and provisions coverage but does not adjudicate fault or apportionment, which remains external law, and a named anchor guarantees a reachable defendant, not a just outcome, in a doctrinal area untested in any jurisdiction.

A022 adds control-supply density observability by territory and domain plus disclosed thin-market handling routes, not a mechanism that creates verifiers: the admissible set of independent, unrelated, affordable fiscalizers can be genuinely empty in a small commune or narrow domain, so supply-density signals — eligible fiscalizers and evidence producers per scope, offer rates, fee levels, and repeat-assignment concentration — attach to the existing Planning Scope, Control Subproject selection, and fiscalization records, and a thin-market indicator names a structural gap before publication rather than at control-package closure. Thin-market handling is an explicit, disclosed country-implementation choice — remote and documentary evidence, cross-territory and pooled assignment, and relaxed-but-disclosed selection under stronger safeguards and longer observation windows — and priced compensation for weak verification per Proposition 2 raises the guarantee rather than lowering the standard. Core v0 records and surfaces control-supply density but cannot conjure qualified verifiers; supply creation through training and accreditation subsidies is country implementation, and in a genuinely empty market a monopolist controller and unavoidably conflicted selection remain unresolved.

A023 gates open-mode deployment; it does not define the amendment rule: C019 and H017 fix the procedural envelope of rule change but decline to specify how a non-tutored open-mode protocol change is decided, and rule-level capture is the highest-leverage attack surface, so no public function may exit into open mode until constitutional-level decision mechanics — who votes, with what weights and quorum, and how constitutional rules differ from operational parameters — are resolved and published. The C019 procedural shell stays binding in every mode; tutored and semi-open pilots run indefinitely under existing Governance Resolution and Review Timeout machinery; when the mechanics are later designed no proposal may define the mechanism that ratifies it, identity, allocation authorization, and the amendment rule are entrenched behind the constitutional tier, and participation and concentration metrics attach to any protocol-change vote. Core v0 records the gate but leaves the mechanics undefined by design, so any premature open-mode deployment would face rule-level capture with no defense, and full constitutional meta-governance remains Extension v1+.

A024 adds salience-bias observability above the protected floor, not preference correction or a welfare function: atomized voluntary allocation inherits the salience, warm-glow, and photogenic-output biases of charitable giving, so above the essential floor abstract, preventive, statistical-life goods can be under-allocated relative to their welfare value, while the non-assignable pool and essential floors (A005) already exclude the catastrophic-underprovision cases. The salience gradient becomes an observable property — funding per category against declared planning-scope need weights across salient-versus-non-salient and preventive-versus-remedial lanes, with underfunded-need indicators in the discovery surface — and default allocation rules route inactive budget by planning priority rather than salience, with any default weighting toward under-allocated non-salient categories a disclosed, versioned protocol choice gated by threshold policy. Core v0 exposes the gradient and routes defaults but does not correct citizen preferences; above the floor honest citizens may keep directing free choice to salient projects, and portfolio rebalancing beyond defaults is Extension v1+.

A025 adds engagement-decay and active-core observability joined to functional transition maturity, not participation quotas: each citizen's monthly allocation is individually small, so by Downsian logic most route it through the cheapest channel and active evaluation decays after launch, leaving a shrinking, self-selected active core displayed as the will of the whole. The mix of manual, profile-driven, delegated, and untouched-default allocation becomes a measured, versioned public property of each cycle, active-core concentration is tracked across cycles, citizen-facing legitimacy language distinguishes deliberate evaluation from default and profile carry-over, and a cadence-fatigue review evaluates the monthly cycle for decay effects, with these signals fed into H054 functional transition maturity. The simulation vindicates the designed-intermediation position — allocation quality is carried by the institutional default anchor conditioned on the informational quality of the weight vector it follows, and quintupling citizen attention moves quality selection by at most ~0.08 — so Core v0 measures and discloses the participation mix but does not mandate engagement, and measurement does not by itself reverse decay: cohort-refresh and sampling remain Extension v1+.

A026 adds declared identity-provider failure modes plus purpose-bound access auditing, not anonymity or a detection engine: the verified-identity baseline (P004) makes the external identity provider the system's deepest dependency, whose compromise, coercion, or misconfiguration can mint Sybil identities or silently exclude citizens, while binding a real identity to every civic act chills complaints against powerful actors. The provider becomes a declared critical dependency with enumerated failure modes — compromise, coercion, outage, wrongful exclusion — and compromise-response procedures including re-verification windows, action freezes, and retroactive audit; every access to act-to-identity linkage is itself an AuditEvent carrying its purpose and checkable by an independent party; protected-identity contexts (C014/C024) are retained with a heightened, adversarial de-anonymization threshold for complaints against executors, authorities, and the operator; and verified identity is preserved as the anti-Sybil baseline rather than replaced by an anonymous formal layer. Core v0 governs the marginal linkage of civic acts through purpose-limited logged access; provider-independent anomaly detection, federation, and substitution remain Extension v1+, and provider compromise remains a single point of failure whose ultimate mitigation is jurisdictional.

A027 adds herding observability, not a visibility redesign: visible funding progress, active-support counts, and delegate follower numbers turn each allocation into social proof that later, weakly-informed citizens rationally imitate, so early and partly arbitrary advantages compound into cascades only loosely correlated with value, with no manipulating operator required. Funding progress stays visible while the platform surfaces the cumulative-advantage dynamics acting on it — funding-velocity-versus-evidence-maturity indicators, funding-source independence signals including the share arriving after high-visibility exposure, discovery-slot concentration observability over "most supported" and "almost funded" ordering, and funding-target closure (H035) retained as the structural cascade truncator with its claim stated honestly. Per [[simulation-results|research/simulation-results.md]] experiment E1, closure lowers concentration and lets roughly 15% more projects complete but does not by itself improve quality selection, which lives in the default anchor rather than the cap. Core v0 measures and bounds herding rather than preventing it; blind early windows, staged reveal, and exposure equalization remain Extension v1+, and visible social proof is intrinsic to a transparent funding surface — a declared limitation.

A028 adds per-territory and per-group outcome observability, not equalization: even with strictly equal per-citizen wallets, allocation outcomes concentrate where social capital, organizational density, and proposal-writing capacity already concentrate, so equal inputs produce unequal territorial and group results, and horizontal equity requires equalization the model does not contain. Core v0 mandates that the distribution of outcomes be measured — funding per capita by comuna as a standing versioned metric, funded-project density against planning-scope need weights, capture share by organized proposers, and an explicit input-equity versus outcome-equity distinction so equal wallets are not read as fair outcomes. Equalization capability is documented as available and unmandated — redistributive and hybrid allocation-amount formulas (H025), planning-scope lanes and the A005 floor-and-lane pattern reused for a territorial or group equity target, the H033 default rule's territorial routing, and preferential supply-side funding of proposal, modeling, and fiscalization support — each a versioned, publicly authorized, auditable country-implementation choice. Core v0 makes the eight-to-one gap measurable for the first time but does not close it; how much to equalize is a political choice, not an architectural one.

A029 strengthens Composite Program dependency coordination without abandoning per-project accountability: decomposing public action into independently funded projects can strand complementarities where one component is worthless without another, and each component still lives or dies by its own funding attempt. A project may declare hard dependencies on other components inside a Composite Program (H051), visible before citizens fund; program-level funding coherence shows which components are funded and unfunded with stranded-complement warnings when a prerequisite funds and its dependents do not; a dependent component's funding window can reference its prerequisite's state rather than expiring blindly against it; and procurement-scale losses are acknowledged as a real cost of modularity. Components keep independent funding attempts — Core v0 makes no all-or-nothing program funding, and conditional or escrowed bundle funding, shared procurement, and shared control packages remain Extension v1+, so complementarities can still strand when prerequisites fund and dependents do not — a declared limitation.

A030 adds pattern-level clientelism indicators while keeping sanction in external law: clientelist machines migrate into new institutions as delegates, participation-support providers, or intermediaries, exchanging allocation behavior for off-platform favors invisible by construction. The platform makes the on-platform half traceable and surfaces repeated patterns over time — repeat broker-beneficiary alignments across cycles, geographic delegation blocs correlated with related-project funding, and participation-support providers whose assisted citizens concentrate on related actors — feeding review, complaint, and verified-discovery paths as leads, never automatic verdicts, through the A014 related-party graph and delegation records rather than a new entity or a hard anti-broker ban. Participation-support projects carry anti-conditioning declarations and independent beneficiary confirmation, and a citizen can revoke a delegation privately for future actions. The off-platform quid pro quo remains invisible and sanction routes to external law; a careful machine trading entirely off-platform and splitting influence across entities remains undetectable from within — a declared limitation.

A031 fixes a motive-neutrality boundary and adds mobilization-pattern observability: allocation can become an identity battleground of tribal signaling, brigade-funding, and boycott campaigns, but expressive, identity-driven allocation is legitimate citizen choice and the platform does not police motive. Citizens' reasons for funding are outside the platform's authority (P002), so no motive-policing and no viewpoint moderation of allocation; material effects stay gated identically regardless of motive — antivalue declarations route through measurement and fiscalization, affected-party evidence and threshold policies bind, and a contested antivalue supports a complaint only if someone files one; funding-bloc correlation (synchronized timing, shared origin, reciprocal brigading) is research and administrative data only, without discounting either side's signals; and funding-target closure (H035) bounds brigade amplitude on any single project. A platform that discounted "tribal" support would arrogate exactly the authority over value P002 denies it, so the fight is kept materially accountable, not suppressed; expressive allocation can still shift spending toward symbolic battlegrounds — a declared limitation.

A032 adds long-horizon lanes and intertemporal observability while keeping the existing shields: monthly cycles reward visible near-term outputs and renewal windows re-expose long-horizon commitments to shifting attention, so hyperbolic discounting tilts new funding away from maintenance, prevention, and intergenerational infrastructure, and future citizens are unrepresented in every funding attempt. Planning Scopes may define long-horizon lanes — maintenance, prevention, infrastructure — whose need-weights flow into the default allocation as a transparent versioned choice; intertemporal allocation is observable through funding share by time-to-benefit horizon and deferred-maintenance indicators; continuity-risk classification (A006) and the non-assignable pool are retained as the structural shields pulling existing obligations into protected renewal paths; and renewal windows are made visible before expiry. Core v0 does not discount-correct citizen preferences or force intergenerational weights, and multi-cycle or endowment lanes insulated from per-cycle attention remain Extension v1+; formation bias above the floor is mitigated only insofar as planning-scope lanes and defaults carry it — where the simulation shows allocation quality lives — a declared limitation tied to the A020 agenda constraint.

A034 adds an allocation-act characterization record on the Allocation Mandate, not a doctrinal construction: the citizen allocation act has no settled legal nature — administrative act, sui generis participation act, or mere preference input — and each characterization implies different review rights, liability, and contestability, so an unstated one imports litigation risk the architecture never designed for, read against a replaced official act whose formal characterization never delivered practical per-decision accountability. Every active Planning Scope's Allocation Mandate additionally records how the citizen allocation act is characterized in the jurisdiction, chosen from a bounded menu, with a pilot default of sui generis participation act carrying vote-like citizen immunity and no individual review right over rule-based reassignments while the complaint path stays unchanged for rule violations; the characterization is disclosed in plain language on the citizen surface before first allocation, a comparator note records how the replaced official act is characterized and reviewed in the pilot jurisdiction, and any change is a higher-authorization decision with versioning and audit events. Core v0 records the jurisdiction's choice and makes it visible but does not construct, adjudicate, or harmonize administrative-law doctrine or grant or deny standing; no characterization of a citizen allocation act has been tested before any court, so the doctrine is untested by definition until a pilot produces a dispute — a declared limitation.

A035 adds an Administrative Capacity Declaration per tutored scope plus capacity-calibrated timeouts and bounded authority-side assistance, not a staffing prescription: tutored operation requires human reviewers to decide admissibility, resolutions, and timeouts within declared windows, and an under-resourced authority produces lapse cascades that read as model failure rather than as an unfunded post, though the review labor already exists today unmeasured and the model re-platforms it under declared windows and public attribution. Every tutored Planning Scope publishes, alongside its Allocation Mandate and Fiscal Commitment Profile, an Administrative Capacity Declaration recording expected review volume, staffing assignment, and per-resolution effort assumptions; timeout policies are calibrated to that declared capacity and never copied between scopes with different declarations; authority-side AI assistance for resolution drafting, admissibility checklists, and window tracking stays inside the C008 boundary where AI assists, the responsible authority decides, and every decision is attributable; review-window performance joins the H054 functional transition maturity metrics; and hours-per-resolution measurement by resolution type is committed as an explicit first-pilot deliverable, the corpus's first empirical staffing data. Core v0 does not model staffing economics or prescribe headcounts — operating capacity is budgeted by the installing authority as country implementation — and a lapsed deadline never produces an anonymous system action but a public Review Timeout Resolution naming the responsible authority; until a pilot measures hours per resolution type every declaration is a public, falsifiable assumption — a declared limitation.
...
- mandate construction, validation, referendum machinery, and formula adjudication beyond the A019 Allocation Mandate record — which, per A037, also carries the enabling-norm record and binding-mode gate — with the platform recording and displaying legal authorization but never generating it;
- distributed roadmap and planning-area construction governance beyond the A020 interim observability, which keeps agenda-setting power visible, versioned, and contestable but does not distribute who constructs Planning Scopes;
...
- liability adjudication — determining who is at fault for third-party harm and in what proportion — beyond the A033 Duty-of-Care Anchor and third-party coverage, which name the answerable legal person and provision coverage before disbursement but leave apportionment to external law;
...
- open-mode voting mechanics — who votes on protocol changes, with what weights, quorum, and constitutional entrenchment — until the A023 gate is resolved and published, with open-mode deployment barred and the C019 procedural shell binding in every mode meanwhile;
- preference correction, welfare functions, and mandated portfolio rebalancing toward non-salient goods beyond the A024 salience-bias observability and disclosed default weighting, which expose the gradient and route inactive budget by planning priority but do not force the abstract good to win;
- participation quotas, engagement targets, and cohort-refresh or sampling mechanisms beyond the A025 engagement-decay and active-core metrics, which measure and disclose the participation mix but do not by themselves reverse decay;
- anonymity and identity-provider replacement — provider-independent anomaly detection, federation, and substitution — beyond the A026 declared failure modes and purpose-bound access auditing, which govern the marginal linkage of civic acts but keep verified identity as the anti-Sybil baseline;
- staged-visibility de-herding — blind early windows, staged reveal, and exposure equalization — beyond the A027 herding observability and funding-target closure, which measure and bound cascades but keep funding progress visible rather than hidden;
- outcome equalization — territorial or group floors, matching, compensatory routing, and redistributive formulas — beyond the A028 per-territory and per-group outcome observability, which makes distributive concentration measurable but leaves how much to equalize to authorized country implementation;
- all-or-nothing program funding — conditional or escrowed bundle funding with automatic return, and shared procurement or control packages across linked projects — beyond the A029 cross-project dependency declarations and stranded-complement warnings, which coordinate composite programs while keeping components' funding attempts independent;
...
- administrative-law doctrine construction and standing adjudication — determining the legal nature of the citizen allocation act and who may contest it — beyond the A034 characterization record, which registers and discloses the jurisdiction's chosen characterization but does not construct, adjudicate, or harmonize the doctrine;
- staffing economics and headcount prescription — sizing review capacity, deployment cost models, and administrative budgets — beyond the A035 Administrative Capacity Declaration and capacity-calibrated timeouts, which make the assumed capacity public and falsifiable but leave capacity provision to country implementation until a pilot measures hours per resolution;
