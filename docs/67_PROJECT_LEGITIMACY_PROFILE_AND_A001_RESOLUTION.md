# Project Legitimacy Profile and A001 Resolution v0
...
A001 is partially founded and does not distort the project when it is read as a warning against confusing funding with legitimacy. The project rejects the distorted conclusion that only centralized authorities can legitimate public action. The accepted resolution is a proportional `Project Legitimacy Profile` for projects whose effects reach beyond ordinary funder preference.
...
Ordinary citizens should not see a legal memo. They should see compact labels such as:
...
The profile prevents dashboards, proponents, delegates, or authorities from using funding closure as a proxy for social approval. Layer 5 should preserve the profile, objections, routes used, source records, and final status.

## Residual risks

- Implementers may still communicate funded projects as socially approved.
- The exact threshold for requiring a profile may vary by country or public function.
- Affected-party voice must not become an automatic veto over every project.
