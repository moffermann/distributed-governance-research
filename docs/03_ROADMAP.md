...
Next review candidates, in order: the audit-institution evidence sweep required by `docs/105` (supreme audit institutions and ex-post evaluation bodies across several countries and political orientations) feeding the S′ parameterization; implementation and execution of E7; the manuscript's v1.7 revision (headline recalibration per E7, in-model qualifiers, the docs/107 exclusion-wording correction, the docs/109 Section 9 restatement, and the round's framing and literature corrections); then the post-manuscript validation program — external expert review using the prepared packets, calibration of the simulation to participatory-budgeting data, and a bounded tutored pilot with the instrumentation the resolutions already require.
