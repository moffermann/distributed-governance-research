...
| Bounded distributed resource allocation `H002` | Core v0 framing / financing boundary | Needed to explain that distributed allocation applies only to the configured assignable public-purpose share, inside active planning scopes, eligibility rules, threshold policies, evidence, fiscalization, phase gates, and audit controls. It preserves the non-assignable common pool and does not treat popularity, funding, or algorithmic ordering as project legitimacy, execution readiness, disbursement approval, or fulfillment proof. |
