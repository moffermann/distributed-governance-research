...
| `FulfillmentEvidenceNeed` | `need_id`, `contract_ref`, `metric_or_claim_ref`, `expected_source_roles`, `required_metadata`, `review_rule`, `status` | Evidence producers may offer to satisfy it. | Contract-matched offers receive higher priority. |
...
| `EvidenceContext` | `context_id`, `evidence_item_ref`, `proposed_context`, `confirmed_context`, `confirmed_by_role_ref`, `effect_scope`, `status` | Prevents complaint evidence, fulfillment evidence, and control evidence from being mixed. | Same material may have multiple reviewed contexts. |
