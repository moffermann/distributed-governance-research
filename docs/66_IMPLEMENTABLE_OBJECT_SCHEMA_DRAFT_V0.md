...
No separate HerdingIndicator entity required in Core v0. A027 herding observability is represented over citizen discovery and project-list ordering (C025), the support-versus-financing separation (H026), the funding-target closure rule (H035), and administrative observability: funding_velocity_vs_evidence_maturity, funding_source_independence including the share arriving after high-visibility exposure, and discovery_slot_concentration, with funding-target closure the structural cascade truncator. Per the simulation, closure lowers concentration and lifts completion breadth but does not by itself improve quality selection, which lives in the default anchor; blind early windows, staged reveal, and exposure equalization remain Extension v1+.
No separate TerritorialEquityMetric entity required in Core v0. A028 outcome observability is represented over the audit trail, PlanningScope, the civic tax wallet and allocation formula (H025), and the default allocation rule (H033): funding_per_capita by comuna, funded_project_density against need weights, capture_share by organized proposers, and an explicit input-equity versus outcome-equity distinction. Equalization — redistributive and hybrid formulas, planning-scope lanes and the A005 floor-and-lane pattern reused for a territorial or group equity target, territorial routing, and supply-side support — is a versioned, publicly authorized, auditable country-implementation choice rather than a Core v0 mandate.
No separate CrossProjectDependency entity required in Core v0. A029 dependency coordination is represented over the existing Composite Program object (H051), project and phase funding attempts, and Planning Scope alignment (H009): hard cross-project dependency declarations (X requires Y) visible before funding, program-level funding-coherence visibility with stranded_complement_warning when a prerequisite funds and its dependents do not, and dependency-aware funding windows. Components keep independent funding attempts; conditional or escrowed bundle funding, shared procurement, and shared control packages remain Extension v1+.
No separate ClientelismIndicator entity and no automatic verdict required in Core v0. A030 pattern-level observability is represented over delegation and the Delegate role (C019), participation-support projects, the related-party relationship graph (A014), delegated-action reports, and the complaint and competent-authority referral paths: repeat broker-beneficiary alignments across cycles, geographic delegation blocs correlated with related-project funding, and support-provider concentration, feeding review and referral paths as leads, with anti-conditioning declarations and independent beneficiary confirmation on participation-support projects. The off-platform quid pro quo remains invisible and sanction routes to external law.
No separate MotiveOrPolarizationRecord entity required in Core v0, and no motive is policed. A031 is a motive-neutrality boundary plus funding-bloc correlation observability over social sovereignty over value (P002), value icons and antivalue-ceiling declarations (H012), support and justified-objection signals (H026), the common-good impact declaration (C022), the funding-target closure rule (H035), and the deliberation surface: material effects stay gated identically regardless of motive, and funding_bloc_correlation (synchronized timing, shared origin, reciprocal brigading) is research and administrative data only. Mobilization-pattern indicators carry no platform effect; viewpoint moderation and polarization scores are excluded on principle.
No separate IntertemporalAllocationMetric entity required in Core v0. A032 is observability plus optional planning-scope lanes over Planning Scope (H009), the monthly allocation cycle (H038), continuity-risk classification (A006), citizen funding and automatic allocation profiles, and administrative observability: long-horizon lane need-weights flowing into the default allocation, funding_share_by_time_to_benefit_horizon, and deferred_maintenance_indicator, with renewal windows surfaced before expiry and the non-assignable pool retained as the structural shield. Discount-correcting preferences, forced intergenerational weights, and multi-cycle or endowment lanes remain Extension v1+.
No separate AllocationActCharacterization entity required in Core v0. A034 is a field set on the PlanningScope Allocation Mandate record: allocation_act_characterization with a bounded menu (administrative_act, sui_generis_participation_act, or preference_input), a pilot default of sui_generis_participation_act with vote-like citizen immunity and no individual review right over rule-based reassignments, comparator_note on the replaced official act, plain-language citizen-surface disclosure before first allocation, and version history with AuditEvents treating any change as a higher-authorization decision, while the complaint path stays unchanged for rule violations. The platform records the jurisdiction's chosen characterization; doctrine construction, standing adjudication, and menu consequences remain external constitutional and administrative law.
No separate AdministrativeCapacityDeclaration entity required in Core v0. A035 is carried on PlanningScope as administrative_capacity_declaration — expected_review_volume, staffing_assignment, and per_resolution_effort with version history — with timeout policies calibrated to it and never copied between scopes with different declarations, authority-side AI assistance inside the C008 boundary where AI assists and the responsible authority decides, review-window performance (decisions in window, lapses, timeout outcomes) joined to the H054 functional transition maturity metrics, and hours_per_resolution measurement by resolution type committed as a first-pilot deliverable. A lapsed deadline produces a public ReviewTimeoutResolution naming the responsible authority; staffing economics, headcount prescription, and deployment cost models remain country implementation.
```
