...
A034 adds an allocation-act characterization record on the Allocation Mandate, not a doctrinal construction. The citizen allocation act has no settled legal nature — administrative act, sui generis participation act, or mere preference input — and each characterization implies different review rights, liability, and contestability, read against a replaced official act whose formal characterization never delivered practical per-decision accountability. Every active Planning Scope's Allocation Mandate records how the act is characterized in the jurisdiction from a bounded menu, with a pilot default of sui generis participation act carrying vote-like citizen immunity and no individual review right over rule-based reassignments while the complaint path stays unchanged for rule violations; the characterization is disclosed in plain language on the citizen surface before first allocation, a comparator note records how the replaced official act is characterized and reviewed, and any change is a higher-authorization decision with versioning and audit events. Core v0 records the jurisdiction's choice and makes it visible but does not construct, adjudicate, or harmonize administrative-law doctrine or grant or deny standing; no characterization of a citizen allocation act has been tested before any court, so the doctrine is untested by definition until a pilot produces a dispute.

A035 adds an Administrative Capacity Declaration per tutored scope plus capacity-calibrated timeouts and bounded authority-side assistance, not a staffing prescription. Tutored operation needs human reviewers to decide admissibility, resolutions, and timeouts within declared windows, and an under-resourced authority produces lapse cascades that read as model failure rather than as an unfunded post, though the review labor already exists today unmeasured. Every tutored Planning Scope publishes, alongside its Allocation Mandate and Fiscal Commitment Profile, an Administrative Capacity Declaration recording expected review volume, staffing assignment, and per-resolution effort assumptions; timeout policies are calibrated to that declared capacity and never copied between scopes with different declarations; authority-side AI assistance for drafting, admissibility checklists, and window tracking stays inside the C008 boundary where AI assists and the responsible authority decides; review-window performance joins the H054 functional transition maturity metrics; and hours-per-resolution measurement by resolution type is committed as an explicit first-pilot deliverable. Core v0 does not model staffing economics or prescribe headcounts — operating capacity is budgeted by the installing authority as country implementation — and a lapsed deadline produces a public Review Timeout Resolution naming the responsible authority, never an anonymous system action; until a pilot measures hours per resolution type every declaration is a public, falsifiable assumption.
...
- administrative-law doctrine construction and standing adjudication for the citizen allocation act beyond the A034 characterization record, which registers and discloses the jurisdiction's chosen characterization but does not construct, adjudicate, or harmonize the doctrine;
- staffing economics, headcount prescription, and deployment cost models beyond the A035 Administrative Capacity Declaration and capacity-calibrated timeouts, which make the assumed capacity public and falsifiable but leave capacity provision to country implementation until a pilot measures hours per resolution;
