...
// --- E8: behavioral participation bridge -------------------------------------
// Pre-registered in research/e8-behavioral-participation-design.md (predictions
// committed before this implementation). Replaces the distributed arms' imposed
// participation constants (d=0.8, pi=0.3) with trajectories generated by the
// behavioral adoption experiment (behavioral-adoption-abm, satellite repository
// github.com/moffermann/distributed-governance-experiments), per the
// author-approved mapping: d_t = 1 - active share; informed = direct actives
// plus delegated weight (the delegation->informed fold); piInformed = plateau
// informed population share. The executor side stays audit-anchored (docs/105):
// OPAQUE / CALIBRATED / VERIFIED are E7's regimes verbatim; the machinery below
// mirrors E7's, generalized only to per-cycle d/pi. The AUDIT cell runs E8 with
// E7's constants and must reproduce E7's Np=200 numbers exactly.
if (args.exp === "e8") {
  const { readFileSync: readE8 } = await import("node:fs");
  const inputsPath = args.inputs ?? new URL("../../research/e8-behavioral-inputs.json", import.meta.url);
  const E8IN = JSON.parse(readE8(inputsPath, "utf8"));
  const { N, CYCLES, ETA, SCARCITY } = CONFIG;
  const PI_BASE = 0.3, KC = 4, SIGMA_C = 0.35, SIGMA_P = 0.10, SLOTS = 6;
  const K_FIXED = 30, K_RATIO = 0.15, POOL = 120, L = 6;
  const SCALES = [10, 20, 40, 200];
  const kScaled = (Np) => Math.max(3, Math.round(K_RATIO * Np));
  ...
  // E7's allocateSelection, generalized to per-cycle participation arrays.
  // With constant arrays it consumes the RNG stream identically to E7.
  const allocateSelection = (world, cfg, seed, salt) => {
    const rng = mulberry32((seed ^ ((salt + 1) * 0x9e3779b9)) >>> 0);
    const { Np, theta, s, target, informedProjs, informedSigs, wOpenRaw, wPlannerFixed, wPlannerScaled } = world;
    const { dArr, piArr, constructor } = cfg;
    const funded = new Float64Array(Np);
    const closeCycle = new Int32Array(Np).fill(-1);
    const nInformed = informedProjs.length;
    const staticW = constructor === "plannerFixed" ? wPlannerFixed
      : constructor === "plannerScaled" ? wPlannerScaled : wOpenRaw;
    let staticOrder = null;
    if (dArr.some((d) => d > 0)) staticOrder = Array.from({ length: Np }, (_, j) => j).sort((a, b) => staticW[b] - staticW[a]);
    for (let t = 0; t < CYCLES; t++) {
      const defaultCount = Math.round(N * dArr[t]);
      const activeCount = N - defaultCount;
      const activeInformed = Math.round(activeCount * piArr[t]);
      const activeUninformed = activeCount - activeInformed;
      for (let c = 0; c < activeInformed && nInformed > 0; c++) {
        const ci = Math.floor(rng() * nInformed);
        const projs = informedProjs[ci], sigs = informedSigs[ci];
  ...
  const execute = (world, port, reg, execSeed) => {
    ...
    const detectRng = mulberry32((execSeed ^ 0xd4) >>> 0);
    const replaceRng = mulberry32((execSeed ^ 0xe5) >>> 0);
    const thr = reg.p * ((1 - reg.a * (1 - reg.r)) + reg.gamma + reg.R);
    const pool = new Array(POOL);
    for (let i = 0; i < POOL; i++) pool[i] = poolRng() < 0.7 ? 0 : 1;
    const jobs = port.jobs;
    let totalBudget = 0, deliveredBudget = 0, officialBudget = 0, value = 0;
  ...
  const meanCI = (arr) => {
    const m = mean(arr);
    const se = sd(arr) / Math.sqrt(arr.length);
    const t = 1.972; // ~t(0.975, df~200)
    return `${m.toFixed(3)} [${(m - t * se).toFixed(3)}, ${(m + t * se).toFixed(3)}]`;
  };
  const constArr = (v) => new Array(CYCLES).fill(v);
  const D_CENTRAL = constArr(1.0), PI_CENTRAL = constArr(PI_BASE);

  const runCell = (Np, behavioralRuns, variant) => {
    const acc = { S: [], Sp: [], A1p: [], A3p: [], A2: [] };
    const ratios = [];
    for (let r = 0; r < RUNS; r++) {
      const seed = BASE_SEED + r;
      for (const brun of behavioralRuns) {
        const world = makeWorld(seed, Np, brun.piInformed);
        let dArr, piArr;
        if (variant === "trajectory") { dArr = brun.d; piArr = brun.pi; }
        else {
          const tail = Math.max(1, Math.round(brun.d.length / 3));
          const dPlateau = mean(brun.d.slice(-tail)), piPlateau = mean(brun.pi.slice(-tail));
          dArr = constArr(dPlateau); piArr = constArr(piPlateau);
        }
        const portS = allocateSelection(world, { dArr: D_CENTRAL, piArr: PI_CENTRAL, constructor: "plannerFixed" }, seed, 0);
        const portSp = allocateSelection(world, { dArr: D_CENTRAL, piArr: PI_CENTRAL, constructor: "plannerScaled" }, seed, 1);
        const portD = allocateSelection(world, { dArr, piArr, constructor: "openStatic" }, seed, 3);
        const execS = (seed * 4 + 1) >>> 0, execSp = (seed * 4 + 2) >>> 0, execD = (seed * 4 + 3) >>> 0;
        const out = {
          S: execute(world, portS, OPAQUE, execS),
          Sp: execute(world, portSp, CALIBRATED, execSp),
        ...
        for (const k of Object.keys(acc)) acc[k].push(out[k].V);
        if (out.Sp.V > 1e-9) ratios.push(out.A2.V / out.Sp.V);
      }
    }
    return { acc, ratios };
  ...
  console.log(`\n# E8 - behavioral participation bridge (paper seeds ${BASE_SEED}..${BASE_SEED + RUNS - 1} x behavioral seeds per population, N=${N}, cycles=${CYCLES})`);
  console.log(`inputs: ${E8IN.generated}, mapping ${E8IN.mapping}`);

  // AUDIT cell: E8 with E7's constants must reproduce E7's Np=200 numbers.
  {
    const e7run = [{ piInformed: PI_BASE, d: constArr(0.8), pi: constArr(PI_BASE) }];
    const { acc, ratios } = runCell(200, e7run, "trajectory");
    console.log(`\n## AUDIT cell (Np=200, d=0.8, pi=0.3, piInformed=0.3 - must equal E7 main table)`);
    console.log(`V(S)=${fmtMS(acc.S)}  V(S')=${fmtMS(acc.Sp)}  V(A1')=${fmtMS(acc.A1p)}  V(A3')=${fmtMS(acc.A3p)}  V(A2)=${fmtMS(acc.A2)}  ratio=${meanCI(ratios)}`);
  }

  for (const [popName, pop] of Object.entries(E8IN.populations)) {
    for (const variant of ["steady", "trajectory"]) {
      console.log(`\n## E8 ${popName} / ${variant} (${pop.seeds} behavioral seeds x ${RUNS} paper seeds per scale)\n`);
      console.log("| Np | V(S) | V(S') | V(A1') | V(A3') | V(A2) | V(A2)/V(S') per-pair | ratio of means |");
      console.log("|---|---|---|---|---|---|---|---|");
      for (const Np of SCALES) {
        const { acc, ratios } = runCell(Np, pop.runs, variant);
        const excl = RUNS * pop.runs.length - ratios.length;
        const note = excl > 0 ? ` (${excl} excl.)` : "";
        console.log(`| ${Np} | ${fmtMS(acc.S)} | ${fmtMS(acc.Sp)} | ${fmtMS(acc.A1p)} | ${fmtMS(acc.A3p)} | ${fmtMS(acc.A2)} | ${meanCI(ratios)}${note} | ${(mean(acc.A2) / mean(acc.Sp)).toFixed(3)} |`);
      }
    }
  }
  process.exit(0);
