...
// LEGACY RUNTIME GUARD — reproduction-only. This retired engine emits D/C and 'x'; it must never feed v1.14.
if (process.env.E4_ALLOW_LEGACY !== '1') {
  console.error('[legacy guard] pre-v1.14 engine (retired D/C framing); NOT the v1.14 evidence path. Set E4_ALLOW_LEGACY=1 to run for v1.12/v1.13 reproduction only.');
  process.exit(2);
}
