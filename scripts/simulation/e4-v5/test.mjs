...
// NULL: both arms near the reference => negligible gap.
{
...
// BOUNDARY: a narrow-central-lead boundary case. Under Core-v0-faithful universal participation the fully-idealized central
// endpoint (PRO_CENTRAL) is the a narrow central win (~ −2.3%), not NO_MYOPIA (which is ~+14%).
{
  const { PRO_CENTRAL } = await import('./scenario-configs.mjs');
  const val = m(PRO_CENTRAL);
  check('FIX boundary (narrow central lead at idealized central endpoint) => |m| small', Number.isFinite(val) && Math.abs(val) < 0.10, `m=${val.toFixed(3)}`);
}
...
check('EMBARGO allows clean percent text', !rejects('m̂ is 45.7% of the reference, parity at 0'));
