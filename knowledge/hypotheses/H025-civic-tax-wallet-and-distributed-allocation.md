...
H025 is aligned with [[../../docs/102_ALLOCATION_ACT_CHARACTERIZATION_AND_A034_RESOLUTION|A034]]: the same Allocation Mandate that records this formula's authorization additionally records how the citizen's allocation act is legally characterized in the jurisdiction — a pilot default of sui generis participation act with vote-like citizen immunity and no individual review right over rule-based reassignments — disclosed in one plain sentence on the citizen surface before first allocation, so the non-withdrawable public allocation right described here carries a stated legal nature rather than an assumed one, while doctrine construction stays external law.

H025 is aligned with [[../../docs/96_TERRITORIAL_OUTCOME_OBSERVABILITY_AND_A028_RESOLUTION|A028]]: because equal per-citizen wallets guarantee an equal right to allocate but not an equal share of where allocation lands, the redistributive, inverse, and hybrid formula options here are exactly the equalization layer A028 documents as available and unmandated, alongside planning-scope lanes, the H033 default rule's territorial routing, and supply-side support in low-capacity territories. Core v0 mandates that per-territory and per-group outcomes be observable — funding per capita by comuna, funded-project density against need weights, and capture share by organized proposers — but leaves how much to equalize as a versioned, publicly authorized country-implementation choice, not an architectural one.
